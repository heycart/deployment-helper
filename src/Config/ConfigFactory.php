...
class ConfigFactory
{
    ...
    {
        $file = EnvironmentHelper::getVariable('HEYCART_PROJECT_CONFIG_FILE', $application->projectConfigFile);

        if ($file === null) {
            $file = Path::join($projectDir, '.heycart-project.yml');

            if (!file_exists($file)) {
                $file = Path::join($projectDir, '.heycart-project.yaml');
            }
        } else {
    ...
    {
        if (EnvironmentHelper::hasVariable('HEYCART_STORE_LICENSE_DOMAIN')) {
            $projectConfiguration->store->licenseDomain = EnvironmentHelper::getVariable('HEYCART_STORE_LICENSE_DOMAIN', '');
        }
