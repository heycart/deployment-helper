...
class AccountServiceTest extends TestCase
{
    ...
    #[Env(name: 'HEYCART_STORE_ACCOUNT_EMAIL', value: 'test@exaple.com')]
    #[Env(name: 'HEYCART_STORE_ACCOUNT_PASSWORD', value: 'password')]
    public function testShopSecretAdd(): void
    {
        $output = $this->createMock(SymfonyStyle::class);
        $output->expects($this->once())->method('info')->with(static::stringContains('Refreshed global shop token to communicate to store.heycart.com'));
    ...
    public function testLoginPayloadChanged(): void
    {
        ...
        $this->expectExceptionMessage('Got invalid response from HeyCart API: {"message":"Invalid credentials"}');
        $service->refresh($output, '6.4.0-dev', 'old.example.com');
    }

    #[Env('HEYCART_STORE_SHOP_SECRET', 'some-key')]
